...
export class Snake extends AcGameObject {
    constructor(info, gamemap) {
        ...
        if (this.id === 1) this.eye_direction = 2;  //  左下角的蛇初始朝上，右上角朝下
    ...
    next_step() { //  将蛇的状态变为下一步
        ...
        if (!this.gamemap.check_valid(this.next_cell)) {    //  下一步操作撞了，失败
            this.status = "die";
        }
    }
    ...
    update_move() {
        ...
        if (distance < this.eps) {  //  走到目标点了
            ...
            if (!this.check_tail_increasing()) {
                console.log("use check_tail_increasing");
                const k = this.cells.length;
